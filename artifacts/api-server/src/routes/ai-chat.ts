/**
 * AI Chat — SSE orchestrator endpoint
 *
 ...
 * Fallback: if ANTHROPIC_API_KEY is missing, the route uses the Nexus
 * single-pass planner so the chat is still functional.
 */
...
router.post("/ai-chat/stream", async (req: Request, res: Response): Promise<void> => {
  ...
  // Plan B: no Anthropic key → single-pass synthesis via Nexus
  if (!process.env.ANTHROPIC_API_KEY) {
    try {
      const { nexusRunRole } = await import("../lib/nexus/llm-router.js");
      emit({ event: "agent_start", data: { agent: "🧠 Planner", description: "Using Nexus fallback (no orchestrator key)" } });
      const result = await nexusRunRole("planner", message, {
        systemPrompt: body.system || "You are a GCC B2B research analyst. Answer directly and cite sources where possible.",
        maxTokens: 2000,
      });
      emit({ event: "agent_done", data: { agent: "🧠 Planner", found: true, summary: `via ${result.provider}` } });
      emit({ event: "intent", data: { plan: `nexus_fallback:${result.provider}` } });
      emit({ event: "token", data: result.text });
      emit({ event: "final", data: { text: result.text } });
    } catch (e) {
      emit({ event: "error", data: { message: "No AI provider configured. Set ANTHROPIC_API_KEY, OPENROUTER_API_KEY, GROQ_API_KEY, or GEMINI_API_KEY." } });
...
// GET /api/ai-chat/status — LLM provider availability
router.get("/ai-chat/status", async (_req: Request, res: Response) => {
  try {
    const { getLLMStatus } = await import("../lib/nexus/llm-router.js");
    const status = getLLMStatus();
    res.json({
      orchestrator: !!process.env.ANTHROPIC_API_KEY ? "claude" : "nexus_fallback",
      ...status,
    });
  } catch {
    res.json({ orchestrator: "unavailable", activeProviders: [] });
  }
});
